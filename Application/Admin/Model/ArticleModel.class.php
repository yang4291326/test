...
class ArticleModel extends Model{
    ...
    public function preview ($type,$id,$order='article.sort, article.id desc') {


        $where['article.status'] = array('EQ', 0);
        $where['article.type']   = array('EQ', $type);
        $where['article.member_id'] = UID;
        $field = array('article.id','article.name','article.remark','article.sort','goods.goods_img');

        $data = M('Article')->alias('article')
            ->join('__SHOP_GOODS__ as goods on article.goods_id = goods.id','LEFT')
            ->field($field)
            ->where($where)
            ->order($order)
            ->select();
    ...
    public function previewDetail($id,$order='detail.sort, detail.id desc') {
        $where['detail.article_id'] = array('eq',$id);
        $field = array('photo_path','content','sort');
        $detail = M('ArticleDetail')->alias('detail')
            ->field($field)
            ->where($where)
            ->order($order)
            ->select();
        ...
        foreach ($detail as $k => $v) {
            $data[$k]['photo_path'] = $v['photo_path'];
            $data[$k]['content'] = nl2br($v['content']);
            $data[$k]['imgType'] = $this->getFileSuffix($v['photo_path']);
