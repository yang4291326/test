    ...
    <div class="tw-layout">
        ...
        <div class="tw-list-top">
            ...
            <div class="tw-tool-bar">
                <a class="tw-tool-btn-add" href="<?php echo U('edit');?>"><i class="tw-icon-plus-circle"></i> 添加 </a>
                <a class="tw-tool-btn-del" onclick="javascript:recycle('chkbId', '确认删除?! 删除后此皮肤将不在此地显示!', true)"> <i class="tw-icon-minus-circle"></i> 批量删除 </a>
            </div>
            <!-- E=添加删除 -->
            <!-- S=高级搜索 -->
            <form action="/index.php/Admin/Skin/index" method="post" id='frmSearch'>
            ...
            <table class="tw-table tw-table-list tw-table-fixed">
                ...
                <?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td>
                                ...
                                <a class="tw-tool-btn-del" onclick="javascript:recycle(<?php echo ($vo['id']); ?>, '确认删除?! 删除后此皮肤将不在此地显示!',true)"><i class="tw-icon-minus-circle"></i> 删除</a>
                            </td>
