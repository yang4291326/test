...
<body>
    ...
    <div class="tw-layout">
        <div class="tw-list-hd">自定义属性列表</div>
        ...
        <div class="tw-list-top">
            <div class="tw-tool-bar">
                ...
                <a class="tw-tool-btn-del" onclick="javascript:recycle('chkbId', '确认删除?!删除后该自定义属性将被删除!')">
                    <i class="tw-icon-minus-circle"></i> 批量删除
                </a>
            </div>
            <form action="/index.php/Admin/CustomAttribute/index" method="post" id='frmSearch'>
                <div class="tw-search-bar">
                    <div class="search-form fr cf">
                        <div class="sleft">
                            <input type="text" name="keywords" class="search-input" value="<?php echo I('keywords', '');?>" placeholder="请输入自定义属性名称">
            ...
            <table class="tw-table tw-table-list tw-table-fixed">
                <thead>
                    <tr>
                        <th width="10" class="row-selected">
                            ...
                            <input class="checkbox check-all" id="check-all" type="checkbox" style="display: none;">
                        </th>
                        <th width="50">序号</th>
                        <th width="20%">名称</th>
                        <th width="5%">类型</th>
                ...
                <tbody>
                    <?php if(!empty($data)): if(is_array($data)): $k = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                                ...
                                <td><?php echo (show_attr_status($vo['attr_type'] )); ?> </td>
                                <td><?php echo ($vo["attr_sort"]); ?> </td>
                                <td><?php echo (show_status($vo['attr_status'] )); ?> </td>
                                <td>
                                    <a class="tw-tool-btn-view" href="<?php echo U('CustomAttribute/edit?id='.$vo['id']);?>">
                                        <i class="tw-icon-desktop"></i> 修改
                                    </a>
                                    <a class="tw-tool-btn-del" onclick="javascript:recycle(<?php echo ($vo['id']); ?>, '确认删除?! 删除后该自定义属性将被删除!')" >
