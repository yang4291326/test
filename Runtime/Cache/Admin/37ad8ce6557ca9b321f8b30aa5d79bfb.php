...
<body>
    ...
    <div class="tw-layout">        
        <!-- S=文章管理 -->
        <div class="tw-list-hd"> 升级信息管理</div>
        ...
        <div class="tw-list-top">
            ...
            <!-- E=添加删除 -->
            <!-- S=高级搜索 -->                        
            <form action="/index.php/Admin/Upgrade/index" method="post" id='frmSearch'>
                <div class="tw-search-bar">
                    <div class="search-form fr cf">
                        <div class="sleft">
                            <input type="text" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入升级名称">
            ...
            <table class="tw-table tw-table-list tw-table-fixed">
                ...
                <tbody>
                    <?php if(!empty($data)): if(is_array($data)): $k = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                                ...
                                <td class="text-left"><?php echo ($vo["version"]); ?></td>
                                <td><?php echo (show_upgrade_status($vo['mode'])); ?></td>
                                <td><?php echo (time_format($vo["update_time"])); ?></td>
                                <td><?php echo ($vo["user_name"]); ?></td>
                                <td>
                                    <a class="tw-tool-btn-add" href="<?php echo U('member?upgrade_id='.$vo['id']);?>"><i class="tw-icon-pencil"></i> 升级用户 </a>
                                    <a class="tw-tool-btn-edit" href="<?php echo U('edit?id='.$vo['id']);?>"><i class="tw-icon-pencil"></i> 修改 </a>
                                    <a class="tw-tool-btn-del" onclick="javascript:recycle(<?php echo ($vo['id']); ?>, '确认删除?! 删除后此文章将不在此地显示!',true)"><i class="tw-icon-minus-circle"></i>删除</a>
                                </td>                                
                            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                        <?php else: ?>
                            <tr><td colspan="8" class="text-center"> aOh! 暂时还没有内容! </td></tr><?php endif; ?>
                </tbody>
                <!-- E=详细信息 -->
            </table>
        </form>
